...
const slides = [
    {
        badge: "10월 한정 캐시백 이벤트",
        title: "최대 93.8만원 혜택",
        desc: "지난달보다 오른 최대 캐시백 금액 확인하세요!",
        stack: [
    ...
    {
        badge: "트래블 · 프리미엄",
        title: "공항 라운지 · 해외 적립 2배",
        desc: "여행자에게 꼭 필요한 혜택을 한 번에.",
        stack: [
...
function renderHero() {
    ...
    dots.innerHTML = slides
        .map(
            (_, i) =>
                `<button class="hero__dot" aria-selected="${i === 0 ? "true" : "false"
                }" aria-label="${i + 1}번째 배너"></button>`
        )
        .join("");
    ...
    if (wrap) {
        wrap.addEventListener("mouseenter", stopHeroAuto);
        wrap.addEventListener("mouseleave", startHeroAuto);
    }
    ...
    if (viewport) {
        ...
        const isOnNav = (t) => t && t.closest?.(".hero__nav") != null;

        const onDown = (e) => {
            if (isOnNav(e.target)) return;
            startX = getX(e);
        };
        const onUp = (e) => {
...
function renderBenefit() {
    ...
    const cardWidth = 170 + 8; // min-width + gap
...
function initSearchModal() {
    ...
    function renderHot() {
        hotWrap.innerHTML = HOT.map((v, i) => `<button class="chip ${i < 2 ? "hot" : ""}" data-q="${v}">${v}</button>`).join("");
    }
    function renderSuggest() {
        suggestWrap.innerHTML = SUGGEST.map(
            (s) => `<div class="suggest-card"><div class="k">${s.k}</div><div class="t">${s.t}</div><div class="e">${s.e}</div></div>`
        ).join("");
    }
...
const CARD_ISSUERS = [
    ...
    "우리카드", "하나카드", "NH농협카드", "IBK기업은행", "BC 바로카드", "네이버페이", "현대백화점"
];
...
let selectedCards = [null, null, null];  // 각 슬롯 선택 상태
...
function saveSelected() {
    try { localStorage.setItem(SELECTED_KEY, JSON.stringify(selectedCards)); } catch { }
}
function loadSelected() {
    try {
        ...
        if (Array.isArray(arr) && arr.length === 3) selectedCards = arr;
    } catch { }
}
...
function openPicker(slotIndex) {
    ...
    // 카드사 칩 렌더
    chipsWrap.innerHTML = CARD_ISSUERS.map((n) => {
        const on = n === pickerIssuer;
        return `<button class="chip ${on ? "on" : ""}" data-issuer="${n}">${n}</button>`;
    }).join("");
...
function renderPickerList() {
    ...
    <div class="picker-item" data-id="${p.id}" role="option">
      ...
      <div>
        ...
        <div class="picker-issuer">${p.issuer}</div>
      </div>
    </div>
  `).join("");
...
function applyCardToSlot(card) {
    ...
    selectedCards[currentSlot] = card;
    renderSlot(currentSlot, card);
    saveSelected();
}
